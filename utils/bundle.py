# Bundles the Fab Glossary contents into a single Markdown file for distribution.
import argparse
import functools
import json
import re
...
def render_page(page: Page, level=0, convert_svgs=False) -> str:
    ...
    content = re.sub(r"/fab-glossary/(?:[^/]+/)?([^/\s]+)", r"#\1", content)
    content = content.replace(
        "(themed-image://",
        "(https://modal-cdn.com/fab-glossary/light-"
    )
    if convert_svgs:
...
def replace_svgs_with_pngs(text: str, out_dir: Path = ROOT / "dist" / "diagrams") -> str:
    """Replace all Fab Glossary diagram SVGs with PNGs."""
    ...
    pattern = r"https://modal-cdn\.com/fab-glossary/[\w\-.]+\.svg"
...
def get_program_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-file", type=Path, default=ROOT / "dist" / "fab-glossary.md", help="Output markdown file."
    )
